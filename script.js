...
const bookmarkhandler =(e)=>{
  ...
  if(savedArray.length){
    savedArray.forEach((location) => {
      ...
      li.className = "cursor-pointer border-gray-400 flex flex-col transition duration-500 ease-in-out transform hover:-translate-y-2 hover:shadow-2xl rounded-2xl border-2 p-4 mb-4   border-red-400"
      const div1 = document.createElement('div');
      div1.className = "cursor-pointer select-none flex flex-1 flex-col items-center p-4 "
...
const renderLocationDetails =(coords,myIcon,location, vaccine,map)=> {
  ...
  .on('click',(e)=>{
    ...
    if(location.properties.appointments_available){
      pTag2.innerHTML = "Appointments are available at this location! <br> Walkins are welcomed."
    }else{
      pTag2.innerHTML = "Walkins are available! <br> Call for Appointment Info."
    }
      ...
      if (location.properties.appointment_vaccine_types) {
        ...
        if ('moderna' in location.properties.appointment_vaccine_types) {
          ...
          renderLocationDetails(coords,myIcon,location,'Moderna(18+)',map)
            ...
            renderLocationDetails(coords,myIcon,location,'Johnson & Johnson(18+)',map)
        }
      }
    }
  }
...
if (navigator.geolocation) {
  navigator.geolocation.getCurrentPosition((position, err) => {
    ...
    legend.onAdd = function(map) {
        ...
        div.innerHTML += "<div style='display:inline-block' class='my__div--icon-moderna'> </div><span> Moderna(18+) </span><br>";
        div.innerHTML += "<div style='display:inline-block' class='my__div--icon-pfizer'> </div><span> Pfizer(12+) </span><br>";
        div.innerHTML += "<div style='display:inline-block' class='my__div--icon-jj'> </div><span> Johnson & Johnson(18+)</span><br>";
        return div;
      };
    legend.addTo(map);
    setMarkers(map);
    ...
    savedList.addEventListener('click',(e)=> {
      ...
      map.setView([position.geometry.coordinates[1],position.geometry.coordinates[0]],20)
